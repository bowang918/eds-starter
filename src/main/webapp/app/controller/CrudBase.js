Ext.define('App.controller.CrudBase', {
	...
	destroyObject: function(record) {
		...
		Ext.Msg.confirm(i18n.attention, this.destroyConfirmMsg(record), function(buttonId, text, opt) {
			if (buttonId === 'yes') {
				...
				store.sync({
					success: function() {
						App.ux.window.Notification.info(i18n.successful, me.successfulDestroyMsg);
					},
	...
	onFormSaveButtonClick: function(button) {
		...
		store.sync({
			success: function(records, operation) {
				App.ux.window.Notification.info(i18n.successful, me.successfulSaveMsg);
			},
			failure: function(records, operation) {
				store.rejectChanges();
			}
