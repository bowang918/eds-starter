Ext.define('App.controller.Viewport', {
	...
	requires: [ 'App.view.UserSettings' ],
	...
	onSettingsButtonClick: function() {
		if (App.user) {
			var userSettingsWindow = Ext.create('App.view.UserSettings');
			userSettingsWindow.down('form').loadRecord(Ext.create('App.model.User', App.user));
	...
	onUserSettingsSaveButtonClick: function(button) {
		...
		userService.updateSettings(form.getForm().getFieldValues(), function(result) {
			if (result.success) {
				win.close();
				App.ux.window.Notification.info(i18n.successful, i18n.settings_saved);
			}
		});
	},
